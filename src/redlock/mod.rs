...
pub use backend::{LocalCacheBackend, LockBackend, RespBackend};
...
#[derive(Clone)]
pub struct Redlock {
    /// Independent lock backends (local `Cache` or remote RESP)
    instances: Vec<Arc<dyn LockBackend>>,
...
impl Redlock {
    /// Create a new Redlock over in-process `Cache` backends (tests / embedded use).
    pub fn new(instances: Vec<Arc<Cache>>) -> Result<Self> {
        Self::with_backends(LocalCacheBackend::wrap_all(instances))
    }

    /// Create Redlock from arbitrary backends (local and/or remote RESP).
    pub fn with_backends(instances: Vec<Arc<dyn LockBackend>>) -> Result<Self> {
        let instance_count = instances.len();

        if instance_count == 0 {
            return Err(Error::InvalidArgument(
                "At least one instance required for Redlock".to_string(),
            ));
        }
    ...
    /// Create a Redlock instance with custom configuration (local caches).
    pub fn with_config(
    ...
    /// Like `with_config` but accepts mixed backends.
    pub fn with_backend_config(
        ...
        let mut redlock = Self::with_backends(instances)?;
        redlock.retry_count = retry_count;
        redlock.retry_delay_ms = retry_delay_ms;
        redlock.clock_drift_factor = clock_drift_factor;
        Ok(redlock)
    ...
    /// - When enabled, uses `backends` if provided (injectible local caches for tests);
    ///   otherwise creates **remote RESP backends** for each address in
    ///   `--redlock-instances` (live Kore/Redis processes required for locks to succeed).
    pub fn from_config(
        config: &Config,
        ...
        let instances: Vec<Arc<dyn LockBackend>> = match backends {
            Some(b) => {
                if b.is_empty() {
                    return Err(Error::InvalidArgument(
            ...
            None => {
                ...
                info!(
                    "Redlock: remote RESP backends for {} instances {:?}",
                    addrs.len(),
                    addrs
                );
                RespBackend::from_addrs(&addrs)
            }
        };

        let mut redlock = Self::with_backend_config(
            instances,
...
".to_string(),
    ...
    fn try_lock(&self, resource: &str, val: Bytes, ttl_ms: u64) -> Result<Lock> {
        ...
        for (idx, instance) in self.instances.iter().enumerate() {
            if instance.try_acquire(&key, &val, ttl_ms) {
                acquired_count += 1;
    ...
    fn unlock_instances(&self, key: &Bytes, expected_val: &Bytes, instance_indices: &[usize]) {
        for &idx in instance_indices {
            if let Some(instance) = self.instances.get(idx) {
                instance.release_if_equal(key, expected_val);
            }
        }
    }
    ...
    pub fn extend(&self, lock: &Lock, additional_ttl_ms: u64) -> Result<()> {
        ...
        for instance in &self.instances {
            if instance.extend_if_equal(&key, &lock.val, additional_ttl_ms) {
                extended_count += 1;
            }
        }
        ...
        if extended_count >= self.quorum {
            ...
            Err(Error::LockExtensionFailed(format!(
                "Could not extend lock on quorum: {}/{}",
                extended_count, self.quorum
            )))
        }
    }
