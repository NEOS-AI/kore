...
impl ClusterState {
    ...
    pub fn from_nodes_conf(
        ...
        // Batch FL: restore live flags when present in the header.
        if let Some(v) = live.require_full_coverage {
            cs.set_require_full_coverage(v);
        }
        if let Some(v) = live.allow_reads_when_down {
            cs.set_allow_reads_when_down(v);
        }
        if let Some(ip) = live.announce_ip {
            cs.set_announce_ip(if ip.is_empty() { None } else { Some(ip) });
        }
        if let Some(port) = live.announce_port {
            cs.set_announce_port(if port == 0 { None } else { Some(port) });
        }
        if let Some(prio) = live.replica_priority {
            cs.set_local_repl_priority(prio);
        }
    ...
    pub fn save_nodes_conf_to(
        ...
        writeln!(f, "# epoch {}", self.current_epoch())
            .map_err(|e| format!("write error: {}", e))?;
        // Batch FL: live flags (comment vars; skipped by node-line parser).
        writeln!(
            f,
        ...
        .map_err(|e| format!("write error: {}", e))?;
        match self.announce_ip() {
            Some(ip) => writeln!(f, "# announce-ip {}", ip)
                .map_err(|e| format!("write error: {}", e))?,
            None => writeln!(f, "# announce-ip")
                .map_err(|e| format!("write error: {}", e))?,
        }
        match self.announce_port() {
            Some(p) => writeln!(f, "# announce-port {}", p)
                .map_err(|e| format!("write error: {}", e))?,
            None => writeln!(f, "# announce-port 0")
                .map_err(|e| format!("write error: {}", e))?,
        }
...
/// Live cluster flags parsed from `nodes.conf` header comments (Batch FL).
///
/// All fields are `None` when the key is absent (caller keeps defaults).
#[derive(Debug, Default, Clone)]
struct NodesConfLiveFlags {
    require_full_coverage: Option<bool>,
    allow_reads_when_down: Option<bool>,
    /// Present when `# announce-ip` line exists (value may be empty → clear).
    announce_ip: Option<String>,
    /// Present when `# announce-port` line exists (`0` → clear).
    announce_port: Option<u16>,
    replica_priority: Option<u32>,
}
...
/// Parse Batch FL header vars (`# key value`). Ignores unknown comment keys.
fn parse_nodes_conf_live_flags(text: &str) -> NodesConfLiveFlags {
    let mut flags = NodesConfLiveFlags::default();
    for line in text.lines() {
        let t = line.trim();
        if !t.starts_with('#') {
            // Stop at first node line (comments after nodes are rare; still scan all).
            continue;
        }
        ...
        match key {
            "require-full-coverage" => {
                if let Some(b) = parse_yes_no(val) {
                    flags.require_full_coverage = Some(b);
                }
            }
            "allow-reads-when-down" => {
                if let Some(b) = parse_yes_no(val) {
                    flags.allow_reads_when_down = Some(b);
                }
            }
            "announce-ip" => {
                flags.announce_ip = Some(val.to_string());
            }
            "announce-port" => {
                if val.is_empty() {
                    flags.announce_port = Some(0);
                } else if let Ok(n) = val.parse::<u16>() {
                    flags.announce_port = Some(n);
                }
            }
            "replica-priority" => {
                if let Ok(n) = val.parse::<u32>() {
                    flags.replica_priority = Some(n);
                }
            }
            _ => {}
        }
...
mod tests {
    ...
    fn nodes_conf_round_trip() {
        ...
        // Pre-FL header (no live-flag vars) keeps defaults.
        assert!(b.require_full_coverage());
        assert!(!b.allow_reads_when_down());
        assert!(b.announce_ip().is_none());
    ...
    fn save_nodes_conf_to_and_autosave() {
        ...
        assert!(body.contains("# require-full-coverage yes"));
        assert!(body.contains("# allow-reads-when-down no"));
        assert!(body.contains("# replica-priority 100"));
    ...
    /// Batch FL: SAVECONFIG / autosave round-trip restores live cluster flags.
    #[test]
    fn nodes_conf_live_flags_round_trip() {
        let dir = std::env::temp_dir().join(format!(
            "kore-liveflags-ut-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
        ...
        let _ = std::fs::create_dir_all(&dir);
        let a = ClusterState::single_node("127.0.0.1", 7000);
        a.set_require_full_coverage(false);
        a.set_allow_reads_when_down(true);
        a.set_announce_ip(Some("10.9.8.7".into()));
        a.set_announce_port(Some(17000));
        a.set_local_repl_priority(42);
        let path = a
            .save_nodes_conf_to(dir.to_str().unwrap())
            .expect("save");
        let body = std::fs::read_to_string(&path).unwrap();
        assert!(body.contains("# require-full-coverage no"));
        assert!(body.contains("# allow-reads-when-down yes"));
        assert!(body.contains("# announce-ip 10.9.8.7"));
        assert!(body.contains("# announce-port 17000"));
        assert!(body.contains("# replica-priority 42"));

        let b = ClusterState::load_or_single_node("127.0.0.1", 7000, dir.to_str().unwrap());
        assert!(!b.require_full_coverage());
        assert!(b.allow_reads_when_down());
        assert_eq!(b.announce_ip().as_deref(), Some("10.9.8.7"));
        assert_eq!(b.announce_port(), Some(17000));
        assert_eq!(b.local_repl_priority(), 42);
        assert_eq!(b.addr(), ("10.9.8.7".into(), 17000));
        assert_eq!(b.my_id(), a.my_id());

        // CONFIG SET → autosave path.
        b.set_nodes_conf_dir(dir.to_str().unwrap());
        b.set_require_full_coverage(true);
        b.set_allow_reads_when_down(false);
        b.set_announce_ip(None);
        b.set_announce_port(None);
        b.set_local_repl_priority(7);
        b.autosave_nodes_conf();
        let c = ClusterState::load_or_single_node("127.0.0.1", 7000, dir.to_str().unwrap());
        assert!(c.require_full_coverage());
        assert!(!c.allow_reads_when_down());
        assert!(c.announce_ip().is_none());
        assert!(c.announce_port().is_none());
        assert_eq!(c.local_repl_priority(), 7);

        let _ = std::fs::remove_dir_all(&dir);
    }

    /// Batch FL: pre-FL nodes.conf without live-flag vars loads with defaults.
    #[test]
    fn nodes_conf_legacy_without_live_flags_keeps_defaults() {
        let a = ClusterState::single_node("127.0.0.1", 7000);
        let text = format!(
            "# Kore cluster nodes.conf generated by CLUSTER SAVECONFIG\n# epoch {}\n{}",
            a.current_epoch(),
