//! Geospatial set storage (Batch FG-3: physical home is [`Cache::key_values`]).
...
impl Cache {
    ...
    pub fn get_or_create_geo_set(&self, key: &Bytes) -> Result<SharedGeoSet> {
        ...
        if let Some(existing) = self.geo_from_key_values(key) {
            return Ok(existing);
        }
