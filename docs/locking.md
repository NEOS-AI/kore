...
## Unified keyspace facade (Batch FG / FG-2 / FG-3)

- Cross-type **TYPE / EXISTS / `key_type`** go through `Cache::get_key_value`
  (string `map` → `key_values`). Prefer that over ad-hoc `contains_key` walks.
...
- **FG-3:** all non-string types are physical `KeyValue` variants in
  `Cache::key_values` (`ShardedKeyMap`). Strings remain on `Cache::map`
  (optional FG-4 merge). Names must not dual-reside across maps.
